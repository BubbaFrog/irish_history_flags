...
local DEFAULT_FLAG = minetest.settings:get('default_flag') or 'ira'
...
local delete_if_orphan = function( self )
	...
	if node.name ~= "irish_history_flags:upper_mast" and node.name ~= "ignore" then
...
minetest.register_entity( "irish_history_flags:wavingflag", {
	...
	on_activate = function ( self, staticdata, dtime )
		...
		for o=1, #objs do
			...
			if lua and self ~= lua and lua.name == "irish_history_flags:wavingflag" then
...
minetest.register_node( "irish_history_flags:upper_mast", {
	...
	paramtype2 = "facedir",
	tiles = { "flags_baremetal.png", "flags_baremetal.png" },
	wield_image = "ihflags_pole_top_inv.png",
	inventory_image = "ihflags_pole_top_inv.png",
	groups = { cracky = 2 },
	sounds = metal_sounds,
	is_ground_content = false,

	drop = {"irish_history_flags:lower_mast 2", "irish_history_flags:upper_mast"},
	...
	on_place = function( itemstack, placer, pointed_thing )
		...
		if (node.name ~= "irish_history_flags:lower_mast" or node2.name ~= "irish_history_flags:lower_mast") then
			return itemstack
		end
		...
		minetest.set_node(pointed_thing.under - vector.new(0, 1, 0), {name = "irish_history_flags:upper_mast", param2 = param2 })
		minetest.set_node(pointed_thing.under, {name = "irish_history_flags:upper_mast_hidden_1"})

		if not (minetest.is_creative_enabled(pn)) then
			itemstack:take_item()
		end

		local def = minetest.registered_nodes["irish_history_flags:upper_mast"]
	...
	after_destruct = function( pos )
		...
		if minetest.get_node( above1 ).name == "irish_history_flags:upper_mast_hidden_1" then
...
minetest.register_lbm({
	name = "irish_history_flags:respawn_flags",
	label = "Respawn flags",
	nodenames = {"irish_history_flags:upper_mast"},
...
minetest.register_lbm({
	name = "irish_history_flags:update_node_meta",
...
if minetest.registered_items[flagpole_material] then
	minetest.register_craft({
		output = "irish_history_flags:lower_mast 3",
		recipe = {
			{flagpole_material},
			{flagpole_material},
			{flagpole_material},
...
minetest.register_craft({
	output = "irish_history_flags:upper_mast",
	recipe = {
...
-- or destructed.
minetest.register_node( "irish_history_flags:upper_mast_hidden_1", {
	drawtype = "airlike",
	pointable = false,
	paramtype = "light",
...
flags.set_flag_at = function( pos, flag_name )
	...
	if node.name ~= "irish_history_flags:upper_mast" then
		return false
	end
